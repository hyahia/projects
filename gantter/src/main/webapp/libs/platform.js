...
function openBlackPopup(url,width,height,onCloseCallBack,iframeId){
  ...
  if(window.name!=iframeId)
    bg.css({backgroundImage:"url('"+contextPath+"/res/img/black_70.png')"});
...
function openAjaxPopup(){
	  ...
	  var bg=$("<div>").attr("id","__blackpopup__");
	  bg.css({position:'fixed',top:"0px",paddingTop:"300px", left:0,width:'100%',height:'100%',  backgroundImage:"url('res/img/black_70.png')"});
	  bg.append("<div id='bwinPopupd' name='bwinPopupd'></div>");
	  bg.bringToFront();

	  var ret=bg.find("#bwinPopupd");
	  ret.css({width:width, height:height,top:10, "-moz-box-shadow":'1px 1px 6px #333333',overflow:'auto',"-webkit-box-shadow":'1px 1px 6px #333333', background: "rgba(255, 255, 255, 0)", margin:"auto" });

	  var bdiv= $("<div>").css({width:width,position:"relative",height:"0px", textAlign:"right", margin:"auto" });

	  var img=$("<img src='res/ajax-loader.gif' style='position:absolute;right:-0px;top:0px;'>");
	  bdiv.append(img);
...
function openSearchPopup(){
	  ...
	  img.click( function(){
		  ...
		  var proposalIDVal = $("#searchTF").val();
		  if (!proposalIDVal) {
			  $("#searchTF").css({border: '1px solid rgb(255, 111, 111)'});
			  return;
		  }
		  ...
		  // Open Ajax loading dialog
		  openAjaxPopup();
		  
		  prof.reset();
		  $.getJSON("/gantter/projects/search/"+proposalIDVal, {CM:"LOADPROJECT",taskId:taskId}, function(response) {
		      prof.stop();
...
function openRTCToJiraPopup(){
	  ...
	  img.click( function(){
		  ...
		  prof.reset();
		  $.getJSON("/gantter/rtcjira/"+proposalIDVal, {taskId:taskId}, function(response) {
		      prof.stop();
		      console.debug(response);
		      ge.loadProject(response);
		      ge.checkpoint(); //empty the undo stack
...
function openReportPopup(){
	  ...
	  var bg=$("<div>").attr("id","__blackpopup__");
	  bg.css({position:'fixed',top:"0px",paddingTop:"250px", left:0,width:'100%',height:'100%',  backgroundImage:"url('res/img/black_70.png')"});
	  bg.append("<div id='bwinPopupd' name='bwinPopupd'></div>");
	  bg.bringToFront();
	  ...
	  var img=$("<img src='res/closeBig.png' style='cursor:pointer;position:absolute;right:-40px;top:5px;' title='close'>");
	  var reloadButton =$("<button style='cursor:pointer;position:absolute;left:420px;right:20px;top:145px;' id='reloadReportBtn' class='button big' title='Load Report'>Reload</button>");
	  var reportTable =$("<table style='position:absolute;text-align:center;left:10px;right:10px;top:10px;' cellspacing='1' cellpadding='0' width='96%' id='reportTable'><tr><th class='gdfCell gdfColHeader' style='width:40px;'>&nbsp;</th><th class='gdfCell gdfColHeader' style='width:80px;'>Off-Track</th><th class='gdfCell gdfColHeader' style='width:80px;'>On-Track</th></tr> <tr><th class='gdfCell gdfColHeader' style='width:80px;'>Development</th><td class='gdfCell gdfColHeader'></td><td class='gdfCell gdfColHeader'></td></tr> <tr> <th class='gdfCell gdfColHeader' style='width:80px;'>QA</th><td class='gdfCell gdfColHeader'></td><td class='gdfCell gdfColHeader'></td></tr> <tr> <th class='gdfCell gdfColHeader' style='width:80px;'>UAT</th><td class='gdfCell gdfColHeader'></td><td class='gdfCell gdfColHeader'></td></tr></table>");
	  var ajaxImg=$("<img src='res/ajax-loader.gif' style='position:absolute;right:210px;top:70px;'>");

	  bdiv.append(img);
	  bdiv.prepend(reportTable);
	  bdiv.prepend(reloadButton);
	  ...
	  reloadButton.click( function(){
		  ...
		  var taskId = $("#taskSelector").val();
		  var prof = new Profiler("loadServerSide");
		  //ajaxImg.css({visibility:'visible',position:'absolute',right:'-250px',top:'50px'});
		  bdiv.prepend(ajaxImg);
		  prof.reset();
		  $.getJSON("/gantter/report/", {taskId:taskId}, function(response) {
		      prof.stop();
			  ...
			  ajaxImg.remove();
			  reportTable.remove();
		      reportTable =$("<table style='position:absolute;text-align:center;left:10px;right:10px;top:10px;' cellspacing='1' cellpadding='0' width='96%' id='reportTable'><tr><th class='gdfCell gdfColHeader' style='width:40px;'>&nbsp;</th><th class='gdfCell gdfColHeader' style='width:80px;'>Off-Track</th><th class='gdfCell gdfColHeader' style='width:80px;'>On-Track</th></tr> <tr><th class='gdfCell gdfColHeader' style='width:80px;'>Development</th><td class='gdfCell gdfColHeader' >"+jqXHR.responseText.split(",")[0]+"</td><td class='gdfCell gdfColHeader' >"+jqXHR.responseText.split(",")[1]+"</td></tr> <tr> <th class='gdfCell gdfColHeader' style='width:80px;'>QA</th><td class='gdfCell gdfColHeader' >"+jqXHR.responseText.split(",")[2]+"</td><td class='gdfCell gdfColHeader' >"+jqXHR.responseText.split(",")[3]+"</td></tr> <tr> <th class='gdfCell gdfColHeader' style='width:80px;'>UAT</th><td class='gdfCell gdfColHeader' >"+jqXHR.responseText.split(",")[4]+"</td><td class='gdfCell gdfColHeader' >"+jqXHR.responseText.split(",")[5]+"</td></tr></table>");
		      bdiv.prepend(reportTable);
		      console.log("error " + textStatus);
		      console.log("incoming Text " + jqXHR.responseText);
		  });
